...
print '<tr class="tablehead"><th>Month</th><th>EUR/USD</th><th>USD/JPY</th><th>GBP/USD</th><th>USD/CHF</th><th>Total Rate</th></tr>';
$f = 0;
while($s1 = mysql_fetch_row($r1))
{
	...
	print '<td>'.$s1[0].$s1[1].'</td>';
	print '<td>'.$s1[2].'</td>';
	print '<td>'.$s1[3].'</td>';
	print '<td>'.$s1[4].'</td>';
	print '<td>'.$s1[5].'</td>';
	//print '<td>'.$s1[5].'</td>';
	print '<td>'.$s1[6]=$s1[2]+$s1[3]+$s1[4]+$s1[5].'</td>';
	print '</tr>';
}
print '</table>';

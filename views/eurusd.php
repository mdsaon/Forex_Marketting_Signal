...
$sql = "select s.id,s.buyPoint,s.pips,s.stopLoss,s.timeFrom,s.timeTill,c.name as currency,ca.name as category,s.sellPoint,s.takeProfit,s.dateFrom,s.dateTill from signal as s,currency as c,category as ca where s.currencyid=c.id and s.categoryid=ca.id and currencyid=4";
$r = mysql_query($sql);
...
while($s = mysql_fetch_row($r))
{
	...
	print '<td>'.$s[10].'</td>';
	print '<td>'.$s[11].'</td>';
	print '<td><a href="?a=eurusde&id='.$s[0].'">Edit</a></td>';

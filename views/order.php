...
where o.userid=u.id and o.packageid=p.id and o.packagetypeid=pk.id and o.paymentid=pt.id  order by id desc";
...
while($s = mysql_fetch_row($r))
{
	...
	print '<td>'.$s[7].'</td>';
	print '<td>'.$s[8].'</td>';
	if(date('Y-m-d H:i:s')>$s[8])
	{
		$sql1="update orderdetails set status='expire' where id='".$s[0]."'";
		mysql_query($sql1);
		$sql2="select status from orderdetails where id='".$s[0]."'";
		$sf=mysql_query($sql2);
		while($rf=mysql_fetch_row($sf))
		{
				print '<td>'.$rf[0].'</td>';
		}
	}
	...
	print '<td><a href="?a=ordere&id='.$s[0].'">Approve</a></td>';
